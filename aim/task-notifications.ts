 ...
 *
 * Refactor: the four free notification functions all threaded `(cwd, teamName, ...)`.
 * They are now methods on `TaskNotifier` (cwd + team bound at construction),
 * with thin function facades re-exported for backward compatibility.
 */

import { Mailbox } from "./mailbox.js";
import { isTerminalStatus, getTasksDir, type TaskItem } from "./types.js";
...
/**
 * Sends task-lifecycle notifications via mailbox for a given (cwd, team).
 * Both context values are bound at construction so callers stop threading
...
export class TaskNotifier {
  ...
  constructor(
    ...
    this.mailbox = new Mailbox(cwd, team);
  }
  ...
  async notifyUnblocked(
    ...
    for (const blockedId of completedTask.blocks) {
      ...
      if (blocked.owner) {
        ...
        await this.mailbox.write(blocked.owner, {
          from: "task-system",
        ...
        if (candidateAgent) {
          ...
          try {
            await this.mailbox.write(candidateAgent, {
              from: "task-system",
              text: JSON.stringify(notif),
              timestamp: new Date().toISOString(),
            });
    ...
    await this.mailbox.write("team-lead", {
      from: "task-system",
  ...
  async nudgeVerification(allTasks: TaskItem[]): Promise<void> {
    ...
    const nudgeState = this.readNudgeState();
    ...
    if (_markNudgeSentFn) {
      ...
      if (target) {
        try {
          await _markNudgeSentFn(this.cwd, this.team, target.id);
        } catch (err) {
  ...
  async notifyCompleted(taskId: string, completedBy: string): Promise<void> {
    const notif: TaskNotification = {
      type: "task_completed",
      taskId,
      completedBy,
  ...
  async notifyAssignment(
    ...
    await this.mailbox.write(agentName, {
      from: "task-system",
      text: JSON.stringify(notif),
      timestamp: new Date().toISOString(),
    });
  ...
  // ── Nudge state file (private) ──

  private readNudgeState(): NudgeState {
    const fp = path.join(getTasksDir(this.cwd, this.team), NUDGE_STATE_FILE);
    try {
  ...
  private writeNudgeState(state: NudgeState): void {
...
// ============================================================================
// Backward-Compatible Functional API (thin facades over TaskNotifier)
// ============================================================================
...
export async function notifyTaskUnblocked(
  ...
  await new TaskNotifier(cwd, teamName).notifyUnblocked(completedTaskId, allTasks, candidateAgent);
}
...
export async function nudgeVerification(
  ...
  await new TaskNotifier(cwd, teamName).nudgeVerification(allTasks);
}
...
export async function notifyTaskCompleted(
  ...
  await new TaskNotifier(cwd, teamName).notifyCompleted(taskId, completedBy);
}
...
export async function notifyTaskAssignment(
  ...
  await new TaskNotifier(cwd, teamName).notifyAssignment(agentName, taskId, subject, assignedBy);
}
